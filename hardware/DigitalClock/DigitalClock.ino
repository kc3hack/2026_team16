...
void buzzerForceLow() {
  if (pwmAttached) { ledcDetach(BUZZ_PIN); pwmAttached = false; }
  pinMode(BUZZ_PIN, OUTPUT);
  digitalWrite(BUZZ_PIN, LOW);
}
void buzzerOn(uint16_t freq_hz) {
  if (freq_hz == 0) { buzzerForceLow(); return; }
  if (!pwmAttached) { ledcAttach(BUZZ_PIN, freq_hz, BUZZ_RES); pwmAttached = true; }
  else { ledcChangeFrequency(BUZZ_PIN, freq_hz, BUZZ_RES); }
  ledcWrite(BUZZ_PIN, dutyValue());
}
...
constexpr Note ALARM_PATTERN[] = {
  {F_SO, SEG_MS}, {F_FA, SEG_MS}, {F_SO, SEG_MS}, {0, SEG_MS},
};
...
void updateSound() {
  ...
  if (dur > 0 && (now - soundStart) < dur) return;
  ...
  if (soundIdx >= soundRepeat) {
    ...
    if (soundRepeatLeft > 0) soundRepeatLeft--;
...
void updateAck() {
  if (!ackPlaying) return;
  uint32_t now = millis();
  uint16_t dur = ACK[ackIdx].ms;
  if (dur > 0 && (now - ackStart) < dur) return;
...
bool checkButtonRepeat(Button& btn) {
  if (!btn.pressed) return false;
  // 長押し判定に達しているか確認
  if (!isButtonLongPressed(btn)) return false;
