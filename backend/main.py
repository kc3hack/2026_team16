...
from apscheduler.schedulers.asyncio import AsyncIOScheduler  # 非同期対応のスケジューラー
...
# 0. WebSocketエンドポイント (デバイス常時接続用)
@app.websocket("/ws/{user_id}")
async def websocket_endpoint(websocket: WebSocket, user_id: int):
    await manager.connect(user_id, websocket)
    try:
        while True:
            # デバイス側からのメッセージ待機（接続維持）
